...
class ConversationService:
    ...
    def __init__(
        ...
        llm_connection_repo: "LlmConnectionRepository" = Depends(lambda: __import__('dataline.repositories.llm_connection').repositories.llm_connection.LlmConnectionRepository()),
    ) -> None:
        ...
        self.llm_connection_repo = llm_connection_repo
    ...
    async def generate_title(self, session: AsyncSession, conversation_id: UUID) -> str:
        ...
        llm_conn = await self.llm_connection_repo.get_default(session)
        if not llm_conn:
            raise UserFacingError("No default LLM connection configured.")
    ...
    async def query(
        ...
        user_info = await self.settings_service.get_user_info(session)
        llm_conn = await self.llm_connection_repo.get_default(session)
        if not llm_conn:
            raise UserFacingError("No default LLM connection configured.")
        ...
        langsmith_api_key = getattr(user_info, "langsmith_api_key", None)

        async for chunk in query_graph.query(
            query=query,
            options=QueryOptions(
                secure_data=secure_data,
                openai_api_key=llm_conn.api_key,  # type: ignore
                openai_base_url=llm_conn.base_url,
