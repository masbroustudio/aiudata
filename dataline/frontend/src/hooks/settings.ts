...
const AVATAR_QUERY_KEY = ["AVATAR"];export function getBackendStatusQuery(options = {}) {
  return queryOptions({
...
export function useGetLlmConnections() {
  const { isSuccess } = useQuery(getBackendStatusQuery());
  return useQuery({
    queryKey: LLM_CONNECTIONS_QUERY_KEY,
    queryFn: async () => {
      const result = await api.listLlmConnections();
      return result ?? [];
    },
    enabled: isSuccess,
  });
...
export function useCreateLlmConnection(options = {}) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (payload: {
      provider: string;
      ...
      base_url?: string;
      is_default?: boolean;
    }) => await api.createLlmConnection(payload),
    onSuccess() {
      enqueueSnackbar({
        variant: "success",
        message: "LLM connection saved",
      });
    },
    onError() {
      enqueueSnackbar({
        variant: "error",
        message: "Error saving LLM connection",
      });
    },
    onSettled() {
