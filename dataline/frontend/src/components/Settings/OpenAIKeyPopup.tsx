...
import { useCreateLlmConnection, useUpdateUserInfo } from "@/hooks";
...
export function OpenAIKeyPopup() {
  ...
  const { mutate: updateUserInfo, isPending: isPendingUserInfo } = useUpdateUserInfo();
  const { mutate: createLlmConnection, isPending: isPendingLlm } = useCreateLlmConnection({
    onSuccess() {
      setIsOpen(false);
    },
  });

  const isPending = isPendingUserInfo || isPendingLlm;
  ...
  function saveApiKey() {
    ...
    // Save user preferences first, then save LLM connection
    updateUserInfo(
      { sentry_enabled: sentryEnabled, analytics_enabled: analyticsEnabled },
      {
        onSuccess() {
          createLlmConnection({
            provider: "openai",
