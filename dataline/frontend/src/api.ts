...
const listLlmConnections = async (): Promise<ILlmConnection[]> => {
  return (
    await backendApi<ILlmConnection[]>({ url: `/api/llm-connections/` })
  ).data;
};

const createLlmConnection = async (data: {
  provider: string;
  model: string;
  api_key?: string;
  base_url?: string;
  ...
  return (
    await backendApi<ILlmConnection>({
